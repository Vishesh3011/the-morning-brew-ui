...
import { NewsContent, NewsDetailsContainer, NewsDetailsContent, NewsDetailsFooter, NewsDetailsHeader, NewsImage, NewsLikeIcon, NewsMid, NewsSmall, NewsTitle } from './NewsDetailsStyles'
    ...
    <NewsDetailsContainer>
       {news &&
        <>
            ...
            <NewsMid>
            {news.published_date && <NewsSmall>Published on {news.published_date.toLocaleString('en-IN', options)}</NewsSmall>}
            {!saved ? 
            <NewsLikeIcon onClick={() => handleSaveNews({newsId})}><FavoriteBorderIcon fontSize='large'/></NewsLikeIcon>
          :
            <NewsLikeIcon onClick={() => handleUnSaveNews({newsId})}><FavoriteIcon fontSize='large'/></NewsLikeIcon>}
            </NewsMid>
            <NewsDetailsContent>
                <NewsImage><img src= {news.image_url}/></NewsImage>
                <NewsContent>{news.summary}</NewsContent>
            </NewsDetailsContent>
            <NewsDetailsFooter>
                <a href = {news.url} target = '_blank'>Tap to Read more</a>
            </NewsDetailsFooter>
        </>
       }
